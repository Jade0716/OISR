...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_path', type=str, default='/16T/liuyuyan/example_rendered_slider_lid',
        ...
        for base_name, (files_0, files_1) in tqdm(paired_files.items(), desc=f'Processing {category}', unit='pair'):
            if len(files_0) == len(files_1) and len(files_0) > 0:
                for f0, f1 in zip(files_0, files_1):
                    ret = sample_and_save_with_flow(model, f0, f1, DATA_PATH, SAVE_PATH, NUM_POINTS,gt_flow, VISUALIZE, 'cuda')
                    if ret == -1:
