...
def get_12_cam_positions(theta_deg: float, distance: float, phi_start: float = -30, phi_end: float = 330.0):
    cam_positions = []
    phis = np.linspace(phi_start, phi_end, num=21)[:-1]

    for phi_deg in phis:
        theta_rad = math.radians(theta_deg)
        phi_rad = math.radians(phi_deg)

        x = math.sin(theta_rad) * math.cos(phi_rad) * distance
        y = math.sin(theta_rad) * math.sin(phi_rad) * distance
        z = math.cos(theta_rad) * distance

        cam_positions.append(np.array([x, y, z]))
...
def get_camera_pos_mat(camera):
    ...
    return K, Rtilt_rot, Rtilt_trl,Rtilt
