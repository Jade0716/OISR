...
def filter_invalid_proposals(
    ...
    val_min_points_per_class: list,
    verbose: bool = False,  # 新增参数，是否打印被筛选提案
) -> Instances:
    ...
    valid_score_mask = score_preds > score_threshold

    min_points_per_proposal = torch.tensor(
        [val_min_points_per_class[label.item()] for label in pt_sem_classes],
        dtype=torch.int32,
        device=proposal_indices.device
    )
    valid_points_mask_per_class = num_points_per_proposal > min_points_per_proposal

    valid_proposals_mask = valid_score_mask & valid_points_mask_per_class

    if verbose:
        print("====== Filtered-out Proposals ======")
        for i in range(valid_proposals_mask.shape[0]):
            if not valid_proposals_mask[i]:
                label = pt_sem_classes[i].item()
                score = score_preds[i].item()
                num_points = num_points_per_proposal[i].item()
                iou = proposals.ious[i].max().item() if proposals.ious is not None else -1
                print(f"Idx {i:<4} | Score: {score:.4f} | Class: {label:<2} | Points: {num_points:<4} | IoU: {iou:.4f} | "
                      f"{'Score too low' if not valid_score_mask[i] else ''}"
                      f"{' & ' if not valid_score_mask[i] and not valid_points_mask_per_class[i] else ''}"
                      f"{'Too few points' if not valid_points_mask_per_class[i] else ''}")

    # 接下来保持原样处理剩下的 proposal：
    ...
    return Instances(
        ...
        npcs_preds=proposals.npcs_preds[valid_npcs_mask] if proposals.npcs_preds is not None else None,
        sem_labels=proposals.sem_labels[valid_points_mask] if proposals.sem_labels is not None else None,
        instance_labels=proposals.instance_labels[valid_points_mask] if proposals.instance_labels is not None else None,
        instance_sem_labels=proposals.instance_sem_labels,
        num_points_per_instance=proposals.num_points_per_instance,
        gt_npcs=proposals.gt_npcs[valid_npcs_mask] if proposals.gt_npcs is not None else None,
        npcs_valid_mask=proposals.npcs_valid_mask[valid_points_mask] if proposals.npcs_valid_mask is not None else None,
        ious=proposals.ious[valid_proposals_mask] if proposals.ious is not None else None,
    )
# def filter_invalid_proposals(
#     proposals: Instances,
#     score_threshold: float,
#     val_min_points_per_class: list,  # 这里是一个列表，对应每个类别的最小点数
# ) -> Instances:
#     score_preds = proposals.score_preds
#     proposal_indices = proposals.proposal_indices
#     num_points_per_proposal = proposals.num_points_per_proposal
#     # if score_preds is not None:
#     valid_proposals_mask = score_preds > score_threshold
#
#     min_points_per_proposal = torch.tensor(
#     [val_min_points_per_class[label.item()] for label in proposals.pt_sem_classes],
#     dtype=torch.int32,
#     device=proposal_indices.device)
#
#     # 过滤掉点数少于最小阈值的提案
#     valid_proposals_mask = valid_proposals_mask & (num_points_per_proposal > min_points_per_proposal)
#
#     # 根据有效的提案生成有效的点
#     valid_points_mask = valid_proposals_mask[proposal_indices]
#
#     # 使用 valid_points_mask 过滤掉无效的提案
#     proposal_indices = proposal_indices[valid_points_mask]
#     _, proposal_indices, num_points_per_proposal = torch.unique_consecutive(
#         proposal_indices, return_inverse=True, return_counts=True
#     )
...
#     proposal_offsets[1:] = num_points_per_proposal.cumsum(0)
#
#     # 根据 npcs_valid_mask 来过滤无效的 NPCs
#     if proposals.npcs_valid_mask is not None:
#         valid_npcs_mask = valid_points_mask[proposals.npcs_valid_mask]
#     else:
#         valid_npcs_mask = valid_points_mask
